...
class propagation:
    def __init__(self,video,index):
        ...
        if(not(index == 1)):
            ...
            self.framesFront = torch.from_numpy(np.array(self.framesFront[::-1])).permute((0, 3, 1, 2))[None,:,:,:,:].cpu()
            self.framesFront = (self.framesFront -127.5)/127.5*CONST_e

        if(not(index == len(video))):
            self.frames = video[index:]
            self.frames = torch.from_numpy(np.array(self.frames)).permute((0, 3, 1, 2))[None,:,:,:,:].cpu()
