...
class VideoInpaintingModel:
    ...
    def __init__(self, ref_length=1, num_ref=-1, neighbor_stride=1):
        ...
        self.model.load_state_dict(torch.load('/home/hai/2022_Project_Team4/Model/E2FGVI/model/model.pth', map_location=self.device))
    ...
    def inference(self, frames, masks):
        ...
        for f in tqdm(range(0, t, self.neighbor_stride)):
            ...
            selected_imgs = imgs[:1, neighbor_ids+ref_ids, :, :, :].to(self.device)
            selected_masks = masks[:1, neighbor_ids+ref_ids, :, :, :].to(self.device)
            with torch.no_grad():
                masked_imgs = selected_imgs * (1 - selected_masks)
                del selected_imgs, selected_masks
